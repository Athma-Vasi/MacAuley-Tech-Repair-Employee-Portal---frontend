...
function DisplaySurveys() {
  ...
  useEffect(() => {
    ...
    async function fetchSurveys(): Promise<void> {
      ...
      const url: URL = urlBuilder({
        ...
        query:
          `${queryBuilderString}${pageQueryString}&newQueryFlag=${newQueryFlag}&totalDocuments=${totalDocuments}`,
      });
  ...
  useEffect(() => {
    ...
    async function submitSurveyResponse() {
      ...
      try {
        ...
        const data: {
          message: string;
          resourceData: [SurveyDocument, UserDocument];
        } = await response.json();
  ...
  useEffect(() => {
    ...
    const [completedSurveys, uncompletedSurveys] = responseData.reduce(
      (acc: [SurveyDocument[], SurveyDocument[]], survey) => {
    ...
    uncompletedSurveys.forEach((survey: SurveyDocument) => {
      displaySurveysDispatch({
      ...
      questions?.forEach(
        (
          ...
          switch (responseInput) {
            case "agreeDisagree": {
              ...
              const description = (
                <Text size="sm" aria-label="polite">
                  {`You have selected: ${value ?? "N/A"}`}
                </Text>
              );

              const radioInputCreatorInfoObj:
                AccessibleRadioGroupInputCreatorInfo = {
                  dataObjectArray: SURVEY_AGREE_DISAGREE_RESPONSE_DATA_OPTIONS,
                  description,
                  key:
                    `${_id}-${surveyIdx}-${questionIdx}-${question}-${responseInput}-${responseKind}`,
                  label: question,
            ...
            case "radio": {
              const dataObjectArray: RadioGroupInputData = responseDataOptions
                .map(
                  (responseDataOption) => ({
            ...
            case "checkbox": {
              const dataObjectArray: CheckboxRadioSelectData =
                responseDataOptions.map(
                  (responseDataOption) => ({
                    value: responseDataOption,
                    label: responseDataOption,
                  }),
              ...
              const checkboxInputCreatorInfoObj:
                AccessibleCheckboxGroupInputCreatorInfo = {
                  dataObjectArray,
                  description,
                  key:
  ...
  const displayCreatedSurveys = uncompletedSurveys.map((survey, idx) => {
    ...
    const stepInError: Set<number> = pagesInError.get(_id) ?? new Set<number>();
    ...
    const createdSubmitButton = returnAccessibleButtonElements([
      {
        buttonLabel: "Submit",
        buttonDisabled: surveySubmissions.get(_id) === undefined ||
          pagesInError.get(_id)?.size !== 0,
    ...
    const createdStepperWrapper = (
      <StepperWrapper
        ...
        setCurrentStepperPosition={displaySurveysAction
