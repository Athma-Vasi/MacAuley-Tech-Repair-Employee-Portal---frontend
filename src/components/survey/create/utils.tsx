...
import type { SurveyResponseInput, SurveyResponseKind } from "../types";
...
const SURVEY_HELP_TEXT = (
  <Flex direction="column" align="center" justify="flex-start" rowGap="xs">
    ...
    <Text>
      ...
      Select the type of response you want to collect for your question from the
      entered options. Choose 'Choose one' for a single response, or 'Choose
      any' for multiple responses, or 'Rating' for a rating.
    </Text>
    <Group w="100%" position="left" pl="sm">
      <Text>
    ...
    <Text>
      <strong>Response data options:</strong>{" "}
      Enter the response data options for your question (currently a maximum of
      7 options per question).
    </Text>
    <Group w="100%" position="left" pl="sm">
      <Text>
...
function makeSurveyStateForStepper(state: SurveyState): SurveyState {
  return Object.entries(state).reduce<SurveyState>(
    (stateAcc, [stateKey, stateValue]) => {
      switch (stateKey) {
        case "questions": {
          ...
          Array.isArray(stateValue)
            ? stateValue.forEach((responseKind, responseKindIdx) => {
              Object.defineProperty(
                stateAcc,
                `responseKinds ${responseKindIdx + 1}`,
                {
                  value: responseKind,
                  ...PROPERTY_DESCRIPTOR,
                },
        ...
        case "responseInputs": {
          Array.isArray(stateValue)
            ? stateValue.forEach((responseInput, responseInputIdx) => {
              Object.defineProperty(
                stateAcc,
                `responseInputs ${responseInputIdx + 1}`,
                {
                  value: responseInput,
                  ...PROPERTY_DESCRIPTOR,
                },
            ...
            ? stateValue.forEach((responseOptions, responseOptionsIdx) => {
              Array.isArray(responseOptions)
                ? responseOptions.forEach(
                  (responseOption, responseOptionIdx) => {
                    Object.defineProperty(
                      stateAcc,
                      `responseOption ${responseOptionsIdx + 1} ${
                        INDEX_ALPHABET_TABLE[responseOptionIdx] ??
                          responseOptionIdx + 1
                      }`,
                      {
