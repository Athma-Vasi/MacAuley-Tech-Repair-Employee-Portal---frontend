...
function Entry({
  ...
  const {
    globalState: { themeObject, padding, actionsDocuments },
  } = useGlobalState();
      ...
      {Object.entries(obj).map(([key, value], index) => {
        ...
        const [createdRepairTicketEditButton] = isRepairTicketSectionInView
          ? returnAccessibleButtonElements([
              {
                buttonLabel: <TbEdit />,
                semanticDescription: `Modify ${key} for username: ${obj.username} and form with id: ${obj._id}`,
        ...
        const [
          ...
          // update request status button
          {
            ...
            semanticDescription: `Modify current request status of ${obj.requestStatus} for username: ${obj.username} and form with id: ${obj._id}`,
        ...
        const displayViewProfileButton =
          key === "viewProfile" ? (
            <Tooltip label={`View profile of ${obj.username ?? obj._id ?? obj.email}`}>
          ...
          key === "edit" ? (
            <Tooltip
              label={`Edit repair note for ${
                obj.customerName ?? obj._id ?? obj.username ?? obj.email ?? obj.brand
              }`}
            >
        ...
        const displayCreatedOpenFileUploadsModalButton =
          key === "fileUploads" || key === "viewFile" ? (
            <Tooltip
              label={
                !fileUploadsData[index]?.fileUploads?.length
...
function DisplayQueryCards({
  ...
  const displayQueryCards = Array.from(groupedByQueryResponseData).map(
    (responseDataTuple) => {
      ...
      const titledGroupedFieldTerm = `${groupedFieldTerm
        .toString()
        .charAt(0)
        .toUpperCase()}${groupedFieldTerm.toString().slice(1)}`;
