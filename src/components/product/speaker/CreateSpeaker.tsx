...
function CreateSpeaker({
  ...
  }, [createProductAction.setIsSpeakerColorValid, createProductDispatch, speakerColor]);

  // ╭─────────────────────────────────────────────────────────────────╮
  //    SPEAKER ADDITIONAL FIELDS
  // ╰─────────────────────────────────────────────────────────────────╯
  useEffect(() => {
    const currentlyUpdatingSpeakerFieldAdditional = speakerFieldsAdditionalMap.get(
      currentlySelectedAdditionalFieldIndex
    );
  ...
  const [createdSpeakerTotalWattageTextInput] = returnAccessibleTextInputElements([
    {
      description: {
  ...
  // error/valid text elements
  const [speakerFrequencyResponseInputErrorText, speakerFrequencyResponseInputValidText] =
    AccessibleErrorValidTextElements({
  ...
  // screenreader accessible text input element
  const [createdSpeakerFrequencyResponseTextInput] = returnAccessibleTextInputElements([
    {
      description: {
  ...
  //    SPEAKER INTERFACE
  // ╰─────────────────────────────────────────────────────────────────╯
  const [createdSpeakerInterfaceSelectInput] = returnAccessibleSelectInputElements([
    {
  ...
  // ╰─────────────────────────────────────────────────────────────────╯
  const [createdAddSpeakerFieldsAdditionalMapButton] = returnAccessibleButtonElements([
    {
      buttonLabel: "Add",
  ...
  const speakerFieldsAdditionalMapKeysErrorValidTextElements: [
    ...
    const [
      ...
      inputText: field,
      isInputTextFocused: areSpeakerFieldsAdditionalMapFocused.get(mapKey)?.[0] ?? false,
      isValidInputText: areSpeakerFieldsAdditionalMapValid.get(mapKey)?.[0] ?? false,
  ...
  const speakerFieldsAdditionalMapValuesErrorValidTextElements: [
    ...
    const [
      ...
      inputText: value,
      isInputTextFocused: areSpeakerFieldsAdditionalMapFocused.get(mapKey)?.[1] ?? false,
      isValidInputText: areSpeakerFieldsAdditionalMapValid.get(mapKey)?.[1] ?? false,
  ...
  const createdSpeakerFieldsAdditionalMapTextInputElements = Array.from(
    ...
    const speakerFieldsAdditionalMapKeysTextInputCreatorInfo: AccessibleTextAreaInputCreatorInfo =
      {
        description: {
          error: speakerFieldsAdditionalMapKeysErrorValidTextElements[mapKey][0],
          valid: speakerFieldsAdditionalMapKeysErrorValidTextElements[mapKey][1],
        },
        inputText: field,
        isValidInputText: areSpeakerFieldsAdditionalMapValid.get(mapKey)?.[0] ?? false,
        label: `Name ${mapKey + 1}`,
    ...
    const speakerFieldsAdditionalMapValuesTextInputCreatorInfo: AccessibleTextAreaInputCreatorInfo =
      {
        description: {
          error: speakerFieldsAdditionalMapValuesErrorValidTextElements[mapKey][0],
          valid: speakerFieldsAdditionalMapValuesErrorValidTextElements[mapKey][1],
        },
        inputText: value,
        isValidInputText: areSpeakerFieldsAdditionalMapValid.get(mapKey)?.[1] ?? false,
    ...
    return (
      <Stack key={`speakerFieldsAdditionalMap-${mapKey}`} pt={padding} w="100%">
        <Group position="apart">
          <Text size="md" weight={600}>{`Additional Speaker field ${mapKey + 1}`}</Text>
          {displayDeleteButton}
        </Group>
        <Group position="apart">
          {createdSpeakerFieldsAdditionalMapKeysTextAreaInput}
  ...
  const displaySpeakerFieldsAdditionalMapButton = (
    <Tooltip
      label={`Add additional Speaker field ${speakerFieldsAdditionalMap.size + 1}`}
    >
