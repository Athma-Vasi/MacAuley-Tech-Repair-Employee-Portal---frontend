...
import { CreateProductAction, MouseSensor, PeripheralsInterface } from "../types";
...
function CreateMouse({
  ...
  }, [createProductAction.setIsMouseButtonsValid, createProductDispatch, mouseButtons]);
  ...
  }, [createProductAction.setIsMouseColorValid, createProductDispatch, mouseColor]);
  ...
  //    MOUSE INTERFACE
  // ╰─────────────────────────────────────────────────────────────────╯
  const [createdMouseInterfaceSelectInput] = returnAccessibleSelectInputElements([
    {
  ...
  // ╰─────────────────────────────────────────────────────────────────╯
  const [createdAddMouseFieldsAdditionalMapButton] = returnAccessibleButtonElements([
    {
      buttonLabel: "Add",
  ...
  const mouseFieldsAdditionalMapKeysErrorValidTextElements: [JSX.Element, JSX.Element][] =
    Array.from(mouseFieldsAdditionalMap).map((keyFieldValue) => {
      ...
      const [
        ...
        inputText: field,
        isInputTextFocused: areMouseFieldsAdditionalMapFocused.get(mapKey)?.[0] ?? false,
        isValidInputText: areMouseFieldsAdditionalMapValid.get(mapKey)?.[0] ?? false,
  ...
  const mouseFieldsAdditionalMapValuesErrorValidTextElements: [
    ...
    const [
      ...
      inputText: value,
      isInputTextFocused: areMouseFieldsAdditionalMapFocused.get(mapKey)?.[1] ?? false,
      isValidInputText: areMouseFieldsAdditionalMapValid.get(mapKey)?.[1] ?? false,
  ...
  const createdMouseFieldsAdditionalMapTextInputElements = Array.from(
    ...
    const mouseFieldsAdditionalMapValuesTextInputCreatorInfo: AccessibleTextAreaInputCreatorInfo =
      {
        description: {
          error: mouseFieldsAdditionalMapValuesErrorValidTextElements[mapKey][0],
          valid: mouseFieldsAdditionalMapValuesErrorValidTextElements[mapKey][1],
        },
        inputText: value,
        isValidInputText: areMouseFieldsAdditionalMapValid.get(mapKey)?.[1] ?? false,
    ...
    return (
      <Stack key={`mouseFieldsAdditionalMap-${mapKey}`} pt={padding} w="100%">
        <Group position="apart">
          <Text size="md" weight={600}>{`Additional Mouse field ${mapKey + 1}`}</Text>
          {displayDeleteButton}
        </Group>
        <Group position="apart">
          {createdMouseFieldsAdditionalMapKeysTextAreaInput}
  ...
  const displayMouseFieldsAdditionalMapButton = (
    <Tooltip label={`Add additional Mouse field ${mouseFieldsAdditionalMap.size + 1}`}>
