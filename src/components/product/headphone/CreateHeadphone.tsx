...
import { CreateProductAction, HeadphoneInterface, HeadphoneType } from "../types";
...
function CreateHeadphone({
  ...
  // ╰─────────────────────────────────────────────────────────────────╯
  useEffect(() => {
    const currentlyUpdatingHeadphoneFieldAdditional = headphoneFieldsAdditionalMap.get(
      currentlySelectedAdditionalFieldIndex
    );
  ...
  // screenreader accessible text input element
  const [createdHeadphoneFrequencyResponseTextInput] = returnAccessibleTextInputElements([
    {
      description: {
  ...
  // screenreader accessible text input element
  const [createdHeadphoneImpedanceTextInput] = returnAccessibleTextInputElements([
    {
      description: {
  ...
  //    HEADPHONE INTERFACE
  // ╰─────────────────────────────────────────────────────────────────╯
  const [createdHeadphoneInterfaceSelectInput] = returnAccessibleSelectInputElements([
    {
  ...
  // ╰─────────────────────────────────────────────────────────────────╯
  const [createdAddHeadphoneFieldsAdditionalMapButton] = returnAccessibleButtonElements([
    {
      buttonLabel: "Add",
  ...
  const headphoneFieldsAdditionalMapKeysErrorValidTextElements: [
    ...
    const [
      ...
      isValidInputText: areHeadphoneFieldsAdditionalMapValid.get(mapKey)?.[0] ?? false,
  ...
  const headphoneFieldsAdditionalMapValuesErrorValidTextElements: [
    ...
    const [
      ...
      isValidInputText: areHeadphoneFieldsAdditionalMapValid.get(mapKey)?.[1] ?? false,
  ...
  const createdHeadphoneFieldsAdditionalMapTextInputElements = Array.from(
    ...
    const headphoneFieldsAdditionalMapKeysTextInputCreatorInfo: AccessibleTextAreaInputCreatorInfo =
      {
        description: {
          error: headphoneFieldsAdditionalMapKeysErrorValidTextElements[mapKey][0],
          valid: headphoneFieldsAdditionalMapKeysErrorValidTextElements[mapKey][1],
        },
        inputText: field,
        isValidInputText: areHeadphoneFieldsAdditionalMapValid.get(mapKey)?.[0] ?? false,
        label: `Name ${mapKey + 1}`,
    ...
    const headphoneFieldsAdditionalMapValuesTextInputCreatorInfo: AccessibleTextAreaInputCreatorInfo =
      {
        description: {
          error: headphoneFieldsAdditionalMapValuesErrorValidTextElements[mapKey][0],
          valid: headphoneFieldsAdditionalMapValuesErrorValidTextElements[mapKey][1],
        },
        inputText: value,
        isValidInputText: areHeadphoneFieldsAdditionalMapValid.get(mapKey)?.[1] ?? false,
    ...
    return (
      <Stack key={`headphoneFieldsAdditionalMap-${mapKey}`} pt={padding} w="100%">
        <Group position="apart">
          <Text size="md" weight={600}>{`Additional Headphone field ${mapKey + 1}`}</Text>
          {displayDeleteButton}
        </Group>
        <Group position="apart">
          {createdHeadphoneFieldsAdditionalMapKeysTextAreaInput}
  ...
  const displayHeadphoneFieldsAdditionalMapButton = (
    <Tooltip
      label={`Add additional Headphone field ${headphoneFieldsAdditionalMap.size + 1}`}
    >
