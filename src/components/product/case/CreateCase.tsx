...
function CreateCase({
  ...
  }, [caseColor, createProductAction.setIsCaseColorValid, createProductDispatch]);
  ...
  // ╰─────────────────────────────────────────────────────────────────╯
  const [createdAddCaseFieldsAdditionalMapButton] = returnAccessibleButtonElements([
    {
      buttonLabel: "Add",
  ...
  const caseFieldsAdditionalMapKeysErrorValidTextElements: [JSX.Element, JSX.Element][] =
    Array.from(caseFieldsAdditionalMap).map((keyFieldValue) => {
      ...
      const [
        ...
        inputText: field,
        isInputTextFocused: areCaseFieldsAdditionalMapFocused.get(mapKey)?.[0] ?? false,
        isValidInputText: areCaseFieldsAdditionalMapValid.get(mapKey)?.[0] ?? false,
  ...
  const caseFieldsAdditionalMapValuesErrorValidTextElements: [
    ...
    const [
      ...
      inputText: value,
      isInputTextFocused: areCaseFieldsAdditionalMapFocused.get(mapKey)?.[1] ?? false,
      isValidInputText: areCaseFieldsAdditionalMapValid.get(mapKey)?.[1] ?? false,
    ...
    return (
      <Stack key={`caseFieldsAdditionalMap-${mapKey}`} pt={padding} w="100%">
        <Group position="apart">
          <Text size="md" weight={600}>{`Additional Case field ${mapKey + 1}`}</Text>
          {displayDeleteButton}
        </Group>
        <Group position="apart">
          {createdCaseFieldsAdditionalMapKeysTextAreaInput}
  ...
  const displayCaseFieldsAdditionalMapButton = (
    <Tooltip label={`Add additional Case field ${caseFieldsAdditionalMap.size + 1}`}>
